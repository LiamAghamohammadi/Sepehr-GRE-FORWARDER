...
║   ╚══════╝╚══════╝╚═╝     ╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝  ║
║                                                     ║
║            SEPEHR GRE installer (NO HAPROXY)         ║
║                                                     ║
╚═════════════════════════════════════════════════════╝
EOF
}
...
iran_setup() {
  ...
  while true; do
    ...
    case "${use_mtu,,}" in
      y|yes)
        ask_until_valid "input your custom mtu for gre (576-1600):" valid_mtu MTU_VALUE
        break
        ;;
      n|no|"")
        MTU_VALUE=""
...
log "GRE${ID} | SIDE=$SIDE | OLD IP=$old_ip | NEW IP=$new_ip"
EOF
  ...
  add_log "Automation Regenerate for GRE${id} (NO HAPROXY)"
...
log "Rebuild OK | GRE${ID} | SIDE=$SIDE | restored from backups (NO HAPROXY)"
EOF
  ...
  add_log "Automation Rebuild for GRE${id} (NO HAPROXY)"
  add_log "Script: ${script}"
...
main_menu() {
  ...
  while true; do
    ...
    echo "1 > IRAN SETUP"
    echo "2 > KHAREJ SETUP"
    echo "3 > Services ManageMent (GRE only)"
    echo "4 > Unistall & Clean"
    echo "5 > Rebuild Automation"
    echo "6 > Regenerate Automation"
    echo "7 > Change MTU"
    echo "0 > Exit"
    echo
    ...
    case "$choice" in
      ...
      5) add_log "Selected: Rebuild Automation"; recreate_automation_mode ;;
      6) add_log "Selected: Regenerate Automation"; recreate_automation ;;
...
ensure_root "$@"
add_log "SEPEHR GRE installer (NO HAPROXY / NO FORWARD)."
main_menu
